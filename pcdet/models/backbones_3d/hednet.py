...
class SEDLayerxiu(spconv.SparseModule):
    ...
    def __init__(self, dim: int, down_kernel_size: list, down_stride: list, num_SBB: list, indice_key, xy_only=False):
        ...
        self.duojiao = nn.ModuleList(
                [post_act_block_sparse_3d(dim, dim, 3, 1, 1, indice_key='spconv1sed', conv_type='subm')]

        )
        
        # self.bn = norm_fn_1d(dim)
        # self.relu = nn.ReLU()
        ...
        for idx in range(1, num_levels):
            ...
            # self.duojiao.append(post_act_block_sparse_3d(dim, dim, 3, 1, 1, indice_key=f'spconv1sed_{idx}', conv_type='subm'))
    ...
    def sparse2dto3d(self,sptensor,yuanh,batch):
        xinzuobiao = torch.cat((sptensor.indices[:,0:1]//yuanh,sptensor.indices[:,0:1]%yuanh,sptensor.indices[:,1:]),dim=1)
        xinsparshape = [yuanh] + sptensor.spatial_shape
        xin = spconv.SparseConvTensor(
            features=sptensor.features,
            indices=xinzuobiao.int(),
            spatial_shape=xinsparshape,
            batch_size=batch
        )
        return xin
    
    def sparse3dto2d(self,sptensor,batch):
        shubatch = sptensor.indices[:,0:1]*sptensor.spatial_shape[0]+sptensor.indices[:,1:2]
        xinzuobiao = torch.cat((shubatch,sptensor.indices[:,2:]),dim=1)
        xin = spconv.SparseConvTensor(
            features=sptensor.features,
            indices=xinzuobiao.int(),
            spatial_shape=sptensor.spatial_shape[1:],
            batch_size=batch*sptensor.spatial_shape[0]
        )
        gao = sptensor.spatial_shape[0]
        return xin,gao



    def forward(self, x,batch_size=None,gao=None):
        feats = []
        cishu =0
        # gaoceshi =gao
        # print(gao)
        for conv in self.encoder:
            ...
            # aa=x.indices.clone() 
            if cishu==0:
            # print('qian',x.features.shape)
                x= self.sparse2dto3d(x,gao,batch_size)
            # ce1 = xin.indices.clone()
            
                x= self.duojiao[cishu](x)
                x,gao= self.sparse3dto2d(x,batch_size)
                # print('ffff')
                # print(len(self.duojiao))
            # print(torch.sum(xin.indices-ce1))
            cishu+=1
            
            # xin = xin.replace_feature(self.bn(xin.features))
            # xin = xin.replace_feature(self.relu(xin.features))
            # print('ffff',gao-gaoceshi)
            # print('hou',x.spatial_shape)
            # print('hou',xin.features.shape)
            #x = x.replace_feature(x.features+xin.features)
            
            # x = x.replace_feature(xin.features)
            
            
            # print(torch.sum(aa-xin.indices))
            # print('fffff',cishu)
            feats.append(x)
...
class SparseHEDNet(nn.Module):
    ...
    def __init__(self, model_cfg, input_channels, grid_size, class_names, voxel_size, point_cloud_range, **kwargs):
        ...
        post_act_block = post_act_block_sparse_2d
        # self.stem = spconv.SparseSequential(
        #     post_act_block(input_channels, 16, 3, 1, 1, indice_key='subm1', conv_type='subm'),

        #     SparseBasicBlock2D(16, indice_key='conv1'),
        #     SparseBasicBlock2D(16, indice_key='conv1'),
        #     post_act_block(16, 32, 3, 2, 1, indice_key='spconv1', conv_type='spconv'),

        #     SparseBasicBlock2D(32, indice_key='conv2'),
        #     SparseBasicBlock2D(32, indice_key='conv2'),
        #     post_act_block(32, 64, 3, 2, 1, indice_key='spconv2', conv_type='spconv'),

        #     SparseBasicBlock2D(64, indice_key='conv3'),
        #     SparseBasicBlock2D(64, indice_key='conv3'),
        #     SparseBasicBlock2D(64, indice_key='conv3'),
        #     SparseBasicBlock2D(64, indice_key='conv3'),
        #     post_act_block(64, sed_dim, 3, (2, 2), 1, indice_key='spconv3', conv_type='spconv'),
        # )
        self.stem1 = spconv.SparseSequential(
            post_act_block(input_channels, 16, 3, 1, 1, indice_key='subm1', conv_type='subm'),

            SparseBasicBlock2D(16, indice_key='conv1'),
            SparseBasicBlock2D(16, indice_key='conv1'),)
        self.jiaohu2 = post_act_block_sparse_3d(16, 32, 3, 2, 1, indice_key='spconv1', conv_type='spconv')

        self.stem2 = spconv.SparseSequential(
            SparseBasicBlock2D(32, indice_key='conv2'),
            SparseBasicBlock2D(32, indice_key='conv2'),
        )
        self.jiaohu3 = post_act_block_sparse_3d(32, 64, 3, 2, 1, indice_key='spconv2', conv_type='spconv')
        self.stem3 = spconv.SparseSequential(
                SparseBasicBlock2D(64, indice_key='conv3'),
            SparseBasicBlock2D(64, indice_key='conv3'),
            SparseBasicBlock2D(64, indice_key='conv3'),
            SparseBasicBlock2D(64, indice_key='conv3'),
        ...
        self.jiaohu4 = post_act_block_sparse_3d(64, sed_dim, 3, (1,2,2), 1, indice_key='spconv3', conv_type='spconv')

        self.sed_layers = nn.ModuleList()
        for idx in range(sed_num_layers):
            layer = SEDLayerxiu(
                sed_dim, sed_down_kernel_size, sed_down_stride, sed_num_SBB,
                indice_key=f'sedlayer{idx}', xy_only=kwargs.get('xy_only', True))
            self.sed_layers.append(layer)
        post_act_block = post_act_block_sparse_3d
        self.transition = spconv.SparseSequential(
    ...
    def to_bev(self, x,ba=None):
        # ceshi3d = self.sparse2dto3d(x,self.sparse_shape[0],ba)

        # yuancoords = torch.cat((x.indices[:,:1]//self.sparse_shape[0],x.indices[:,:1]%self.sparse_shape[0],x.indices[:,1:]),dim=1)
        # # print(yuancoords.shape)
        # # print(type([self.sparse_shape[0]]+x.spatial_shape))
        # # print(type(x.spatial_shape))
        # # print(type(self.sparse_shape[0]))
        # x = spconv.SparseConvTensor(
        #     features=x.features,
        #     indices=yuancoords.int(),
        #     spatial_shape=[self.sparse_shape[0]]+x.spatial_shape,
        #     batch_size=ba
        # )
        # x= self.sparse2dto3d(x,)
        # print(torch.sum(x.features-ceshi3d.features))
        # print(torch.sum(x.indices-ceshi3d.indices))
        # print(ceshi3d.spatial_shape)
    ...
    def forward(self, batch_dict):
        ...
        # ceshi = spconv.SparseConvTensor(
        #     features=voxel_features,
        #     indices=voxel_coords.int(),
        #     spatial_shape=self.sparse_shape,
        #     batch_size=batch_size
        # )
        # ceshi2d = self.sparse3dto2d(ceshi,batch_size)
        # print(voxel_coords[:4,:])
        xincoords = torch.cat((voxel_coords[:,:1]*self.sparse_shape[0]+voxel_coords[:,1:2],voxel_coords[:,2:]),dim=1)
        # print(torch.sum(xincoords[:,1:]-voxel_coords[:,2:]))
        # print(xincoords.shape)
        x = spconv.SparseConvTensor(
            features=voxel_features,
            indices=xincoords.int(),
            spatial_shape=self.sparse_shape[1:],
            batch_size=batch_size*self.sparse_shape[0]
        )

        # print(torch.sum(xincoords-ceshi2d.indices))
        # print(torch.sum(x.features-ceshi2d.features))
        # print('dui',x.spatial_shape)
        # print('yan',ceshi2d.spatial_shape)
        # print(ceshi2d.batch_size - x.batch_size,'hha')

        # print(x.spatial_shape)
        x = self.stem1(x)
        x= self.sparse2dto3d(x,self.sparse_shape[0],batch_size)
        x= self.jiaohu2(x)
        x,gao= self.sparse3dto2d(x,batch_size)
        x= self.stem2(x)
        x= self.sparse2dto3d(x,gao,batch_size)
        ...
        x=self.jiaohu4(x)
        x,gao = self.sparse3dto2d(x,batch_size)
        # print('qian',x.spatial_shape)
        for layer in self.sed_layers:
            x = layer(x,batch_size,gao)
        # print('hou',x.spatial_shape)
        x = self.sparse2dto3d(x,gao,batch_size)

        x = self.to_bev(x,batch_size)
